    ...
    def _init_obstacles(self):
        ...
        for i in range(10):
            self.obstacles.append(
                {'c': np.array([start_x + i * 35, start_y + i * 20], dtype=np.float64), 'r': 40, 'drift': False})
        ...
        for i in range(10):
            self.obstacles.append(
                {'c': np.array([200. + i * 60, 400. + random.uniform(-80, 80)], dtype=np.float64), 'r': 30,
                 'drift': True})
        for i in range(8):
            self.obstacles.append(
                {'c': np.array([800. + random.uniform(-40, 40), 300. + i * 70], dtype=np.float64), 'r': 30,
                 'drift': True})
        for _ in range(15):
...
class Executor:
    ...
    def step(self, action):
        ...
        va_vec = np.array(
            [self.propulsion_speed * math.cos(current_ori), self.propulsion_speed * math.sin(current_ori)])
...
class Agent:
    ...
    def load(self, path):
        if os.path.exists(path):
            ...
            Prog.log("WARN", f"Model not found: {path}")
